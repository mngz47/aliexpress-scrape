...
//async
function AliexpressProductScraper(productId, feedbackLimit) {
  ...
  //await
   page.goto(`https://www.aliexpress.com/item/${productId}.html`);
                           //await
  const aliExpressData =  page.evaluate(() => runParams);
  ...
  //await
  page.goto(descriptionUrl);
                              //await
  const descriptionPageHtml =  page.content();
  ...
  /** Fetch the adminAccountId required to fetch the feedbacks */
                          //await
  const adminAccountId = page.evaluate(() => adminAccountId);
  //await 
 browser.close();
